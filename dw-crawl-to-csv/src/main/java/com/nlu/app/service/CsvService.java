...
public class CsvService {
  ...
  public boolean writeCrawlHtmlToCsvFile(String csvPath) {
    ...
    try (FileOutputStream fos = new FileOutputStream(csvPath);
      ...
      String nameHtml, imgHtml, infoHtml, priceHtml;
      int count = 0;
      for (WebElement product : products) {
        try {
          ...
          // Ghi dữ liệu của sản phẩm vào file CSV
          csvWriter.writeNext(new String[]{nameHtml, imgHtml, infoHtml, priceHtml});
          count++;
        } catch (Exception e) {
          e.printStackTrace(); // Bỏ qua sản phẩm bị lỗi
        }
      }
      ...
      // 5.6 Đóng trình duyệt sau khi hoàn thành
      driver.quit();
    }
  }


  public int csvHtmlToCsvData(String csvHtmlPath, String csvDataPath)
    ...
    try (CSVReader csvReader = new CSVReader(new FileReader(csvHtmlPath));
      ...
      while ((nextLine = csvReader.readNext()) != null) {
        ...
        if (nextLine.length < 4) {
          System.out.println(
              "Skipping row due to insufficient columns: " + String.join(", ", nextLine));
          continue;
        }
        ...
        String price = docPrice.select(".product__price--show")
            .text();  // Adjust selector as needed
    ...
    while (true) {
      try {
        ...
        wait.until(ExpectedConditions.not(
            ExpectedConditions.attributeContains(showMoreButton, "class", "is-loading")));
        wait.until(ExpectedConditions.not(
            ExpectedConditions.attributeContains(showMoreButton, "class", "is-large")));
