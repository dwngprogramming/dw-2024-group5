...
BEGIN
    ...
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
		...
		-- 2. Xóa data cũ trong bảng data_cleaning
    ...
    read_loop: LOOP
        ...
        -- 3.3. Preprocessing daily_price với trường hợp có giá
        ...
        END IF;
				...
				-- 3.4. Preprocessing compatibility với chuột của Apple
				...
				-- 3.5. Preprocessing compatibility với các trường NULL
				IF daily_compatibility = 'None' THEN
						SET daily_compatibility = 'Windows';
				END IF;
				
				-- 3.6. Preprocessing connection dựa trên tên (Nếu connection là None)
				...
				-- 3.7. Preprocessing connection 2.4G to receiver
				IF LOCATE(LOWER('2.4G'), LOWER(daily_connection)) OR LOCATE(LOWER('2.4 GHz'), LOWER(daily_connection)) THEN
						SET daily_connection = 'USB Receiver';
				END IF;
				
				-- 3.8. Kiểm tra và chuyển đổi các giá trị None hoặc chuỗi rỗng thành NULL
