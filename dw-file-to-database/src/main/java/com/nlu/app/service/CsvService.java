...
public class CsvService {
    ...
    public int[] csvDataToTempDatabase(String csvPath) throws IOException, CsvValidationException {
        int rowsFile = 0;
        int rowsInserted = 0;

        System.out.println("Deleting data from staging.cp_daily...");
        // Xóa tất cả dữ liệu trong table staging.cp_daily hiện tại
        databaseService.deleteAllRowsFromCpDaily();
        System.out.println("Deleting data from staging.cp_daily completed.");

        // Sử dụng BufferedReader để cải thiện hiệu suất đọc
        try (FileReader fr = new FileReader(csvPath, StandardCharsets.UTF_8);
             BufferedReader br = new BufferedReader(fr);
             CSVReader csvReader = new CSVReader(br)) {

            String[] values;

            // Bỏ qua dòng đầu tiên do là header
            csvReader.readNext();

            while ((values = csvReader.readNext()) != null) {
                boolean insertedSuccess = databaseService.saveOneRowToCpDaily(values);
                if (insertedSuccess) {
                    System.out.println("Inserted row: #" + values[0] + ", Product Name: " + values[1]);
                    ++rowsInserted;
                } else {
                    System.err.println("Failed to insert row: #" + values[0] + ", Product Name: " + values[1]);
                }
                ++rowsFile;
            }
            return new int[]{rowsFile, rowsInserted};
        }
    }
